...
def retrieve(query_ids, doc_ids, similarity, top_k=None):
    """
    ...
    for i in tqdm(range(len(query_ids)), desc="Creating run"):
        ...
        docs_scores = list(zip(doc_ids, query_sim))
...
def embed_bge(model, docs, queries, doc_ids, query_ids, reuse_embeddings, path):
    """
...
def retrieve_top_k(model, query, doc_ids, path, top_k=5):
    """
    Embed the queries and documents using the BAAI embeddings models and compute the similarity between queries and documents.
    Calls the retrieve function.
    ...
    """
    embedding_query = model.encode(query, max_length=MAX_QUERY_LEN)['dense_vecs']

    embeddings_docs = np.load(path)

    # Compute similarities
    similarity = compute_similarity(torch.tensor(embedding_query, dtype=torch.float32), torch.tensor(embeddings_docs, dtype=torch.float32))
    
    run = retrieve(['qid_1'], doc_ids, similarity, top_k)['qid_1']
